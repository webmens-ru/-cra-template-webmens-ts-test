import type { TField, TFilterDates } from "../../components/filter/types";
import type { IDataItem } from "../../components/select";
...
export const getDateResponse = (value: string[]): PostFilterResponse<string>[] => {
  ...
  switch (value[0] as TFilterDates) {
    ...
    case "quarter":
      ...
      let yearQuarterStart = year;
      let yearQuarterFinish = year;
      let nextMonthQuarterStart;
      let nextMonthQuarterFinish;

      if (value[1] === '1') {
        nextMonthQuarterStart = 1;
        nextMonthQuarterFinish = 4;
      } else if (value[1] === '2') {
        nextMonthQuarterStart = 4;
        nextMonthQuarterFinish = 7;
      } else if (value[1] === '3') {
        nextMonthQuarterStart = 7;
        nextMonthQuarterFinish = 10;
      } else if (value[1] === '4') {
        nextMonthQuarterStart = 10;
        nextMonthQuarterFinish = 1;
        yearQuarterFinish++;
      }

      return [
        { operator: ">=", value: `${yearQuarterStart}-${nextMonthQuarterStart}-1` },
        { operator: "<", value: `${yearQuarterFinish}-${nextMonthQuarterFinish}-1` }
      ]
    case "year":
      return [
    ...
    case "nextMonth":
      let yearMaybeNextStart = year;
      let yearMaybeNextFinish = year;
      let nextMonthStart = month + 1;
      let nextMonthFinish = month + 2;
      if (month === 11) {
        yearMaybeNextFinish++;
        nextMonthFinish = 1;
      }
      ...
      return [
        { operator: ">=", value: `${yearMaybeNextStart}-${nextMonthStart}-1` },
        { operator: "<", value: `${yearMaybeNextFinish}-${nextMonthFinish}-1` }
      ]
    default:
      return [];
  }
